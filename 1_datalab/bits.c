...
int leftBitCount(int x) {
    ...
    // special when there is only one/two 1
    x >>= 30;
    res += (x >> 1) & (x + 3);
...
int satMul3(int x) {
    int dx = x + x;
    int tx = dx + x;
    int isOK = (((x ^ dx) >> 31) | ((dx ^ tx) >> 31)) + 1;
    int overflowAns = (~x >> 31) + (1 << 31);
    return ((~isOK + 1) & tx) + ((~!isOK + 1) & overflowAns);
}
/*
...
int isGreater(int x, int y) {
    int isDiffSign = (x ^ y) >> 31;
    int sameSignAns = !(((y + ~x + 1) >> 31) + 1);
    int diffSignAns = (x >> 31) + 1;  // x >= 0 > y
    // A new way to realize "if" sentence
    return ((~!isDiffSign + 1) & sameSignAns) +
           ((~isDiffSign + 1) & diffSignAns);
}
/*
...
int subOK(int x, int y) {
    int isSameSignNot = (x ^ y) >> 31;
    // Always OK if (x, y) has same sign
    // Different sign: OK if (x, x - y) has same sign
    int diffSignAnsNot = (x ^ (x + ~y + 1)) >> 31;
    return !(isSameSignNot & diffSignAnsNot);
}
/*
...
int trueFiveEighths(int x) {
    int sign = x >> 31;
    // if x = 5, 7 mod 8, (x & 5 = 5 in other words), the answer should plus 1
    // this can ONLY solve the positive situation
    int pos = (x >> 1) + (x >> 3) + !((x & 5) ^ 5);
    // another fix on x < 0: minus 1 when x = 0 mod 8
    int neg = pos + 2 + ~!(x & 7);
...
unsigned float_half(unsigned uf) {
    ...
    // the half of the frac --- shift
    unsigned halfFrac = frac >> 1;
    if (frac & 1 & halfFrac)
        halfFrac += 1;

    if (exp == 0xff) {
        return uf;  // Special: NaN & inf
    }
    if (exp) {
        // Normalized
        if (exp != 1) {  // just let exp -= 1
            return (sign + ((exp - 1) << 23) + frac);
        } else {  // Norm -> Denorm
            return (sign + 0x400000 + halfFrac);
        }
    } else {
        // Denormalized
...
unsigned float_i2f(int x) {
    unsigned y, ux, exp, test, decimal, integer, res, temp;
    ...
    if (x) {
        ...
        if (exp <= 23) {
            ...
            // precision loss
            integer = ux >> (exp - 23);
            decimal = (ux & (0xffffffffu >> (55 - exp)));
            test = 1 << (exp - 24);
            temp = integer + 1;
            if (decimal > test)
                integer = temp;
            if ((decimal == test) & integer)
                integer = temp;
            if (integer & 0x1000000) {
                res += 0x800000;
                integer >>= 1;
            }
        }
        return res + (integer & 0x7fffff);
    }
    return 0;
}
/*
...
int float64_f2i(unsigned uf1, unsigned uf2) {
    ...
    int exp = (uf2 << 1) >> 21;
    int frac = 0x40000000 + ((uf2 & 0xfffff) << 10) + (uf1 >> 22);
    if (exp >= 1054)
        return 0x80000000;
    if (exp <= 1022)
        return 0;
