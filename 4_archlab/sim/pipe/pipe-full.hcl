...
####################################################################
#																   #
#	Here's my code for the pipeline instruction.				   #
# ---------------------------------------------------------------- #
#	1. iaddq V rB:												   #
#	Used to add a constant value to a register.					   #
#	Just like that in the SEQ.									   #
#	However, if V == 3267, then it is a special case,			   #
#	which will not set the condition code.						   #
# ---------------------------------------------------------------- #
#	2. jm V, rB:												   #
#	Not really what the instruction is supposed to do.			   #
#	It used to jump to a certain address.						   #
#	But now, It is used to check whether %rB is positive or not,   #
#	if so, make %rax += 1; else, do nothing.					   #
#   And here, V is of no use, just a placeholder.				   #
#																   #
####################################################################
...
wordsig RRAX	 'REG_RAX'	     # General Purpose Register RAX
...
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, IIADDQ, IJM } : D_rB;
...
word d_dstE = [
	D_icode in { IRRMOVQ, IOPQ, IIADDQ, IIRMOVQ} : D_rB;
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
	D_icode in { IJM } : RRAX;
...
word aluA = [
	E_icode in { IRRMOVQ, IOPQ, IJM } : E_valA;
...
word aluB = [
	...
	E_icode in { IJM } && (E_valB > 0) : 1;
	E_icode in { IJM } && (E_valB <= 0) : 0;
	# Other instructions don't need ALU
];
...
bool set_cc = (E_icode == IJM || (E_icode == IIADDQ && E_valC != 3267) || E_icode == IOPQ) &&
