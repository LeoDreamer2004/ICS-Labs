...
long long gen_need_valC()
{
    return ((if_id_next->icode) == (I_IRMOVQ) || (if_id_next->icode) == 
      ...
      (if_id_next->icode) == (I_JMP) || (if_id_next->icode) == (I_CALL) || 
      (if_id_next->icode) == (I_IADDQ) || (if_id_next->icode) == (I_JM));
}
...
long long gen_d_srcA()
{
    return (((if_id_curr->icode) == (I_RRMOVQ) || (if_id_curr->icode) == 
        ...
        (if_id_curr->icode) == (I_JM)) ? (REG_RAX) : (REG_NONE));
}
...
long long gen_d_srcB()
{
    return (((if_id_curr->icode) == (I_ALU) || (if_id_curr->icode) == 
        ...
        (if_id_curr->icode) == (I_IADDQ) || (if_id_curr->icode) == (I_JM))
...
{
    return (((if_id_curr->icode) == (I_RRMOVQ) || (if_id_curr->icode) == 
        (I_ALU) || (if_id_curr->icode) == (I_IADDQ) || (if_id_curr->icode)
         == (I_IRMOVQ)) ? (if_id_curr->rb) : ((if_id_curr->icode) == 
...
long long gen_aluA()
{
    return (((id_ex_curr->icode) == (I_RRMOVQ) || (id_ex_curr->icode) == 
        (I_ALU) || (id_ex_curr->icode) == (I_JM)) ? (id_ex_curr->vala) : (
...
long long gen_aluB()
{
    return (((id_ex_curr->icode) == (I_RMMOVQ) || (id_ex_curr->icode) == 
        ...
        (I_IRMOVQ)) ? 0 : (((id_ex_curr->icode) == (I_JM)) & (
          (id_ex_curr->valb) > 0)) ? 1 : (((id_ex_curr->icode) == (I_JM)) & (
          (id_ex_curr->valb) <= 0)) ? 0 : 0);
}
...
{
    return ((((((id_ex_curr->icode) == (I_JM)) | (((id_ex_curr->icode) == 
                (I_IADDQ)) & ((id_ex_curr->valc) != 3267))) | (
            (id_ex_curr->icode) == (I_ALU))) & (!((mem_wb_next->status) == 
